...
export default function SubTabNavigation({ activeSubTab, onSubTabChange }: SubTabNavigationProps) {
  const subTabs = [
    ...
    { key: 'post', label: 'Post & Schedule', icon: <Instagram className="h-4 w-4" /> },
        ...
        {subTabs.map((subTab) => {
          ...
          return (
            ...
            >
              ...
              {/* Active indicator */}
              <div className={`absolute bottom-0 left-0 right-0 h-0.5 bg-teal-500 transition-all duration-300 ${
                isActive ? 'opacity-100' : 'opacity-0'
              }`} />
              {/* Hover indicator */}
              <div className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gray-300 transition-all duration-300 ${
