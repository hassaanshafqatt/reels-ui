...
export async function POST(
  ...
  try {
    ...
    if (!reelTypeData.is_active) {
      return NextResponse.json(
        { error: `Reel type is not active: ${type}` },
        { status: 400 }
      );
    }

    // Determine whether custom audio is allowed
    const allowCustomAudioGlobally = adminSettingsOperations.getBoolean(
      'allow_custom_audio_globally',
      true
    );
    const isCustomAudioAllowed =
      allowCustomAudioGlobally && reelTypeData.allow_custom_audio !== false;
    ...
    const payload = {
      ...
      minCaptionLength: reelTypeData.min_caption_length || 10,
      maxCaptionLength: reelTypeData.max_caption_length || 100,
      includeAuthor:
        reelTypeData.include_author !== undefined
          ? reelTypeData.include_author
          : true,
      timestamp,
      type,
      ...
      try {
        ...
        if (!externalResponse.ok) {
          ...
          if (externalResponse.status === 413) {
            errorMessage =
              'The audio file is too large for the external service. Please use a smaller audio file (recommended: under 5MB).';
          } else if (externalResponse.status === 415) {
            errorMessage =
              'The audio file format is not supported. Please use MP3, WAV, or M4A format.';
        ...
        return NextResponse.json({
          ...
          jobId,
          message:
            reelTypeData.message ||
            `${reelTypeData.title} reel generation started successfully`,
          type,
          category,
    ...
    return NextResponse.json(
      {
        ...
        details:
          'This reel type requires an external webhook URL to be configured',
        jobId,
        type,
        category,
      },
